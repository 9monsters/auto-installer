#!/bin/bash
# 文件名：install-openssl.sh
# 在安装install-apache.sh时被调用，独立安装时要先挂载网络磁盘

scriptBaseDir="/data/InitScript"
colorFile="${scriptBaseDir}/color-string.sh"
nowTime=`date '+%Y%m%d%H%M%S'`

setupFile=/mnt/autotestsvr/Document/ServerTestFile/SetupFiles/openssl-0.9.8c.tar.gz
...
tar -zxf "${setupFile}" -C "${extractDir}"
cd "${setupDir}"
cd ..
make clean
# 预编译
./config --prefix="${linkDir}" shared zlib-dynamic enable-camellia
# 检测预编译结果
ec="${?}"
if [ "${ec}" -ne 0 ];then
