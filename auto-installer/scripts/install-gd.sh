...
setupFile=/mnt/autotestsvr/Document/ServerTestFile/SetupFiles/gd-2.0.33.tar.gz
...
make clean
# 预编译
./configure --prefix="${linkDir}" --with-freetype=/usr/local/freetype --with-jpeg=/usr/local/jpeg --with-png=/usr/local/libpng/lib --with-libiconv-prefix=/usr/local/libiconv
# 检测预编译结果
ec="${?}"
if [ "${ec}" -ne 0 ];then
